...
import {
  ...
  generateAnsweredPrayerHTML,
  generateApprovedPrayerHTML,
  generateApprovedUpdateHTML,
...
function mockAdminSettingsForBroadcast(
  testAccountEmail: string | null,
  error: unknown = null
) {
  return {
...
describe("EmailNotificationService", () => {
  ...
  it("getTemplate returns template data when present", async () => {
    ...
    mockSupabase.client.from = vi
      .fn()
      .mockReturnValue(makeFromQuery({ data: tpl, error: null }));
  ...
  it("getTemplate returns null on error", async () => {
    mockSupabase.client.from = vi
      .fn()
      .mockReturnValue(
        makeFromQuery({ data: null, error: { message: "fail" } })
      );
  ...
  it("applyTemplateVariables replaces placeholders", () => {
    ...
    const out = service.applyTemplateVariables(content, {
      name: "X",
      missing: "",
    });
  ...
  it("sendEmail calls supabase function and errors on failure", async () => {
    ...
    await expect(
      service.sendEmail({ to: "a@b", subject: "s" })
  ...
  it("sendEmail throws when the function returns success false", async () => {
    mockSupabase.client.functions.invoke.mockResolvedValue({
      data: { success: false, error: "bad payload" },
      error: null,
    });
    await expect(
      service.sendEmail({ to: "a@b", subject: "s" })
  ...
  it("sendEmailToAllSubscribers calls supabase function and errors on failure", async () => {
    ...
    await expect(
      service.sendEmailToAllSubscribers({ subject: "s" })
    ).resolves.toBeUndefined();
  ...
  it("sendEmailToAllSubscribers throws when the edge function returns a failure payload", async () => {
    mockSupabase.client.functions.invoke.mockResolvedValue({
      data: { success: false, error: "Queue failed" },
      error: null,
    });
  ...
  it("sendApprovedPrayerNotification uses fallback when template missing", async () => {
        ...
        return {
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              eq: vi
                .fn()
                .mockResolvedValue({ data: mockSubscribers, error: null }),
            }),
          }),
        };
      }
    ...
    const enqueueSpy = vi
      .spyOn(service as any, "enqueueEmail")
      .mockResolvedValue(undefined);
  ...
  it("sendApprovedPrayerNotification logs queue and trigger failures", async () => {
    ...
    vi.spyOn(service as any, "enqueueEmail").mockRejectedValueOnce(
      new Error("Queue failed")
    );
    vi.spyOn(service as any, "triggerEmailProcessor").mockRejectedValueOnce(
      new Error("Trigger failed")
    );

    await expect(
      service.sendApprovedPrayerNotification({
        title: "T",
        description: "D",
        requester: "R",
        prayerFor: "PF",
  ...
  it("sendAccountApprovalNotification fetches admins via directQuery and calls helper", async () => {
    ...
    const spy = vi
      .spyOn(service as any, "sendAccountApprovalNotificationToEmail")
      .mockResolvedValue(undefined as any);
  ...
  it("sendAccountApprovalNotificationToEmail proceeds when template exists and sends email", async () => {
    ...
    const tpl = {
      subject: "Approve {{firstName}}",
      html_body: "<p>{{approveLink}}</p>",
      text_body: "t",
    } as any;
    // stub getTemplate to return account_approval_request template when called
    vi.spyOn(service, "getTemplate").mockImplementation(async (key: string) =>
      key === "account_approval_request" ? (tpl as any) : null
    );
    const spySend = vi
      .spyOn(service as any, "sendEmail")
      .mockResolvedValue(undefined as any);
  ...
  it("sendRequesterApprovalNotification sends when template present", async () => {
    const tpl = {
      subject: "Your prayer {{prayerTitle}}",
      html_body: "<b>{{prayerTitle}}</b>",
      text_body: "t",
    } as any;
  ...
  it("sendUpdateAuthorApprovalNotification returns early when no email", async () => {
    const spy = vi.spyOn(service as any, "sendEmail");
    await service.sendUpdateAuthorApprovalNotification({
      prayerTitle: "Prayer",
      content: "Update text",
      author: "John",
      authorEmail: "",
  ...
  it("sendUpdateAuthorApprovalNotification sends when template present", async () => {
    const tpl = {
      subject: "Update Approved: {{prayerTitle}}",
      html_body: "<b>{{prayerTitle}}</b> - {{updateContent}}",
      text_body: "t",
    } as any;
    vi.spyOn(service, "getTemplate").mockResolvedValueOnce(tpl as any);
    const spySend = vi
  ...
  it("sendAdminNotification sends to admins when configured and calls helper for each admin", async () => {
    ...
    const spyHelper = vi
      .spyOn(service as any, "sendAdminNotificationToEmail")
      .mockResolvedValue(undefined as any);
  ...
  it("sendAccountApprovalNotification calls sendPushToAdmins with account approval payload", async () => {
    ...
    const spyHelper = vi
      .spyOn(service as any, "sendAccountApprovalNotificationToEmail")
      .mockResolvedValue(undefined as any);
    await service.sendAccountApprovalNotification(
      "user@test.com",
      "Jane",
      "Doe"
    );
  ...
  it("getEmailBaseUrl returns the current origin for non-localhost http origins", () => {
    ...
    vi.stubGlobal("window", {
      location: { origin: "https://prayer.example.com" },
    });
  ...
  it("sendAccountApprovalNotification handles admin query errors gracefully", async () => {
    ...
    await expect(
      service.sendAccountApprovalNotification("user@test.com", "Jane", "Doe")
    ).resolves.toBeUndefined();
  });
  ...
  it("sendAccountApprovalNotificationToEmail catches sendEmail failures", async () => {
    ...
    vi.spyOn(service as any, "sendEmail").mockRejectedValueOnce(
      new Error("Send failed")
    );

    await expect(
      (service as any).sendAccountApprovalNotificationToEmail(
        "user@test.com",
        "Jane",
        "Doe",
        "",
  ...
  it("sendAccountApprovalNotification catches unexpected query errors", async () => {
    mockSupabase.directQuery.mockRejectedValue(
      new Error("Unexpected query failure")
    );
  ...
  it("sendAdminNotification handles push notification failures gracefully", async () => {
    ...
    mockPushNotification.sendPushToAdmins.mockRejectedValueOnce(
  ...
  it("sendDeniedPrayerNotification catches sendEmail failures", async () => {
    ...
    await expect(
      service.sendDeniedPrayerNotification({
        title: "Prayer",
        description: "Desc",
        requester: "Requester",
  ...
  it("sendDeniedUpdateNotification catches sendEmail failures", async () => {
    ...
    await expect(
      service.sendDeniedUpdateNotification({
        prayerTitle: "Prayer",
        content: "Update",
        author: "Author",
  ...
  it("sendUpdateAuthorApprovalNotification catches sendEmail failures", async () => {
    ...
    await expect(
      service.sendUpdateAuthorApprovalNotification({
        prayerTitle: "Prayer",
        content: "Update",
        author: "Author",
        authorEmail: "a@test.com",
  ...
  it("sendAdminNotification catches unexpected helper failures", async () => {
    ...
    vi.spyOn(
      service as any,
      "sendAdminNotificationToEmail"
    ).mockRejectedValueOnce(new Error("Helper failed"));
  ...
  it("sendApprovedUpdateNotification logs queue failures and trigger failures", async () => {
    ...
    await expect(
      service.sendApprovedUpdateNotification({
        prayerTitle: "Prayer",
        prayerDescription: "",
        content: "Update",
        author: "Author",
  ...
  it("sendRequesterApprovalNotification catches sendEmail failures", async () => {
    ...
    await expect(
      service.sendRequesterApprovalNotification({
        title: "Prayer",
        description: "Desc",
        requester: "Requester",
        requesterEmail: "r@test.com",
...
describe("EmailNotificationService - Additional Logic", () => {
  ...
  describe("Email Content Generation", () => {
    it("should generate approval link in content", async () => {
      ...
      const result = service.applyTemplateVariables(content, {
        approveLink: "https://app.com/approve/123",
      });
    ...
    it("should include prayer title in subject", () => {
      ...
      const result = service.applyTemplateVariables(subject, {
        prayerTitle: "Peace",
      });
    ...
    it("should include requester name in body", () => {
      ...
      const result = service.applyTemplateVariables(body, {
        requester: "John Doe",
      });
    ...
    it("should format prayer description", () => {
      ...
      const result = service.applyTemplateVariables(desc, {
    ...
    it("should include update content", () => {
      ...
      const result = service.applyTemplateVariables(content, {
        updateContent: "Much improved, thank you!",
      });
  ...
  describe("Admin push notifications", () => {
    it("should call sendPushToAdmins when sendAdminNotification has admins", async () => {
      ...
      expect(mockPushNotification.sendPushToAdmins).toHaveBeenCalledWith(
        expect.objectContaining({
          title: "Test",
          data: expect.objectContaining({ type: "prayer" }),
        })
      );
    });
  });
  ...
  describe("EmailNotificationService - Additional Coverage - Queuing & Notifications", () => {
    ...
    describe("Email Enqueueing", () => {
      it("should enqueue email successfully", async () => {
        ...
        await expect(
          service.enqueueEmail("user@test.com", "test_template")
        ).resolves.toBeUndefined();
    ...
    describe("Approved Prayer Notifications", () => {
      it("should send approved prayer notification with current status", async () => {
        ...
        const enqueueSpy = vi
          .spyOn(service as any, "enqueueEmail")
          .mockResolvedValue(undefined);
        vi.spyOn(service as any, "triggerEmailProcessor").mockResolvedValue(
          undefined
        );
      ...
      it("should handle empty subscriber list gracefully", async () => {
        ...
        await expect(
          service.sendApprovedPrayerNotification(payload)
        ).resolves.toBeUndefined();
      });
            ...
            return {
              select: vi.fn().mockReturnValue({
                eq: vi.fn().mockReturnValue({
                  eq: vi
                    .fn()
                    .mockResolvedValue({ data: subscribers, error: null }),
                }),
              }),
            };
          }
      ...
      it("should queue appLink with filter=answered when prayer status is answered", async () => {
        ...
        expect(enqueueSpy.mock.calls[0][2].appLink).toContain(
          "filter=answered"
        );
      });
      ...
      it("should trigger email processor after queueing", async () => {
        ...
        const triggerSpy = vi
          .spyOn(service as any, "triggerEmailProcessor")
          .mockResolvedValue(undefined);
    ...
    describe("Approved Update Notifications", () => {
      ...
      it("should handle update notification with missing subscribers gracefully", async () => {
        ...
        await expect(
          service.sendApprovedUpdateNotification(payload)
        ).resolves.toBeUndefined();
      });
    });
    ...
    describe("Account Approval Notifications", () => {
      ...
      it("should handle missing admin list gracefully", async () => {
        ...
        await expect(
          service.sendAccountApprovalNotification(
            "user@test.com",
            "John",
            "Doe"
          )
    ...
    describe("Template Caching and Retrieval", () => {
      it("should cache template after first fetch", async () => {
        ...
        mockSupabase.client.from = vi
          .fn()
          .mockReturnValue(makeFromQuery({ data: template, error: null }));
      ...
      it("should handle null template gracefully", async () => {
        mockSupabase.client.from = vi
          .fn()
          .mockReturnValue(makeFromQuery({ data: null, error: null }));
      ...
      it("should handle concurrent template requests", async () => {
        mockSupabase.client.from = vi
          ...
          .mockReturnValue(
            makeFromQuery({
              data: { subject: "Test", html_body: "HTML", text_body: "Text" },
              error: null,
            })
          );
    ...
    describe("HTML Content Generation", () => {
      it("should generate requester approval HTML", () => {
        ...
        const html = generateRequesterApprovalHTML(
          payload,
          service.getEmailBaseUrl()
        );
      ...
      it("should generate denied prayer HTML", () => {
        ...
        const html = generateDeniedPrayerHTML(
          payload,
          service.getEmailBaseUrl()
        );
        expect(html).toBeDefined();
      ...
      it("should generate denied update HTML", () => {
        ...
        const html = generateDeniedUpdateHTML(
          payload,
          service.getEmailBaseUrl()
        );
        expect(html).toBeDefined();
      ...
      it("should generate approved prayer HTML", () => {
        ...
        const html = generateApprovedPrayerHTML(
          payload,
          service.getEmailBaseUrl()
        );
        expect(html).toBeDefined();
      ...
      it("should generate answered prayer HTML", () => {
        ...
        const html = generateAnsweredPrayerHTML(
          payload,
          service.getEmailBaseUrl()
        );
        expect(html).toBeDefined();
      ...
      it("should generate approved update HTML", () => {
        ...
        const html = generateApprovedUpdateHTML(
          payload,
          service.getEmailBaseUrl()
        );
        expect(html).toBeDefined();
      ...
      it("should generate admin notification prayer HTML", () => {
        ...
        const html = generateAdminNotificationPrayerHTML(
          payload,
          "https://example.com/approve?code=123"
        );
        expect(html).toBeDefined();
        expect(typeof html).toBe("string");
      });
      ...
      it("should generate admin notification update HTML", () => {
        ...
        const html = generateAdminNotificationUpdateHTML(
          payload,
          "https://example.com/approve?code=123"
        );
        expect(html).toBeDefined();
    ...
    describe("Email Processor Triggering", () => {
      ...
      it("should handle edge function error gracefully", async () => {
        ...
        await expect(
          (service as any).triggerEmailProcessor()
        ).resolves.toBeUndefined();
      });
    ...
    describe("Subscriber Welcome Notifications", () => {
      ...
      it("should handle error in welcome notification gracefully", async () => {
        ...
        await expect(
          service.sendSubscriberWelcomeNotification("subscriber@test.com")
        ).resolves.toBeUndefined();
      });
    ...
    describe("Welcome Email HTML Generation", () => {
      it("should generate welcome email HTML", () => {
        const html = generateWelcomeEmailHTML(service.getEmailBaseUrl());
        expect(html).toBeDefined();
        expect(typeof html).toBe("string");
    ...
    describe("Admin Notification HTML Generation", () => {
      ...
      it("should include admin link in prayer notification HTML", () => {
        ...
        const html = generateAdminNotificationPrayerHTML(payload, adminLink);
      ...
      it("should generate deletion admin notification HTML", () => {
        ...
        const html = generateAdminNotificationDeletionHTML(
          payload,
          "https://example.com/admin"
        );
        expect(html).toBeDefined();
      ...
      it("should have consistent styling across all admin notification types", () => {
        ...
        const prayerHtml = generateAdminNotificationPrayerHTML(
          prayerPayload,
          "https://example.com/admin"
        );
        const updateHtml = generateAdminNotificationUpdateHTML(
          updatePayload,
          "https://example.com/admin"
        );
        const deletionHtml = generateAdminNotificationDeletionHTML(
          deletionPayload,
          "https://example.com/admin"
        );
    ...
    describe("Admin Notification Email Sending", () => {
      ...
      it("should handle error in admin notification gracefully", async () => {
        ...
        await expect(
          (service as any).sendAdminNotificationToEmail(
            payload,
            "admin@test.com"
          )
        ).resolves.toBeUndefined();
