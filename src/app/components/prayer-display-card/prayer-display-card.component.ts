...
@Component({
  ...
  template: `
    ...
    <div *ngIf="prayer" class="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-h-full overflow-y-auto">
      <!-- Prayer For -->
      <div class="mb-6">
        <div class="text-2xl font-semibold mb-2 text-blue-600 dark:text-blue-300">Prayer For:</div>
        <div class="text-5xl font-bold leading-tight text-gray-900 dark:text-gray-100">{{ prayer.prayer_for }}</div>
      </div>

      <!-- Description -->
      <div class="mb-6">
        <div class="text-3xl leading-relaxed text-gray-800 dark:text-gray-100">{{ prayer.description }}</div>
      </div>

      <!-- Meta Info -->
      <div class="flex justify-between items-center mb-1 text-xl text-gray-700 dark:text-gray-300 flex-wrap gap-4">
        <div>
      ...
      <div *ngIf="prayer.prayer_updates && prayer.prayer_updates.length > 0" 
        ...
        <div class="text-2xl font-semibold mb-4 text-gray-900 dark:text-gray-100">
        ...
        <div class="space-y-4">
          <div *ngFor="let update of getRecentUpdates()" 
            ...
            <div class="text-xl text-gray-800 dark:text-gray-200">{{ update.content }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div *ngIf="prompt" class="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-h-full overflow-y-auto">
      <!-- Type Badge -->
      <div class="mb-6">
        <span class="inline-block px-5 py-2 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 rounded-full text-xl font-semibold border border-yellow-300 dark:border-yellow-700">
          {{ prompt.type }}
        </span>
      </div>
      ...
      <!-- Title -->
      <div class="mb-6">
        <div class="text-5xl font-bold leading-tight text-gray-900 dark:text-gray-100">{{ prompt.title }}</div>
      </div>

      <!-- Description -->
      <div class="mb-6">
        <div class="text-3xl leading-relaxed text-gray-800 dark:text-gray-100 whitespace-pre-wrap">{{ prompt.description }}</div>
      </div>
    </div>
  `,
  styles: [`
