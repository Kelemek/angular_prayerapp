...
import {
  ...
  GitHubIssueConfig,
} from "../../services/github-feedback.service";
    ...
    >
      ...
      >
        ...
        <app-admin-section-loading
          message="Loading GitHub feedback settings…"
        />
        } @else {
        ...
        <form (ngSubmit)="submitSettings()" class="space-y-6">
          ...
          >
            <p class="text-sm text-amber-800 dark:text-amber-200">
              <strong>Security Note:</strong> Your GitHub token is encrypted in
              our database. Never share your token with anyone.
            </p>
          </div>
        ...
        <div
          class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-3 mt-4"
          role="alert"
          aria-live="assertive"
          aria-atomic="true"
        >
...
export class GitHubSettingsComponent implements OnDestroy {
  ...
  async submitSettings(): Promise<void> {
    ...
    try {
      const success = await this.githubFeedbackService.saveGitHubConfig(
        this.config
      );
