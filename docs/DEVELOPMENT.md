...
- **Practice session shell**: [`memorization-practice-session.component.ts`](src/app/components/memorization-practice-session/memorization-practice-session.component.ts) extends [`memorization-practice-session-facade.ts`](src/app/lib/memorization-practice-session-facade.ts) (~560 lines: lifecycle, handlers, `finishPracticeSession`). Shared state and view getters live in [`memorization-practice-session-facade-base.ts`](src/app/lib/memorization-practice-session-facade-base.ts). Orchestration `run*` modules — listen, scroll, passage, round — take `MemorizationPracticeSessionFacadeBase`; child panels bind explicit inputs for volatile OnPush fields (practicing panel passes tokens, round/hint state, reorder slots, recite metrics; `session` remains for handlers and facade `ViewChild` delegation). Unit tests: shared [`memorization-practice-session.spec-setup.ts`](src/app/components/memorization-practice-session/memorization-practice-session.spec-setup.ts) plus `core`, `hydrate-strict`, `coverage`, and `recite` spec files in the same folder.
