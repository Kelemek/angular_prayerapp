...
- **Scheduling (Supabase)**: pg_cron job **`invoke-dispatch-user-reminders`** (`*/15 * * * *` UTC) POSTs to [`dispatch-user-reminders`](../supabase/functions/dispatch-user-reminders/index.ts), which invokes this function **first**, then memorization hourly, then per-item reminders (sequential; avoids PostgREST stampede). Migration [`20260914183000_dispatch_user_reminders.sql`](../supabase/migrations/20260914183000_dispatch_user_reminders.sql). See [SETUP.md](SETUP.md).
...
- **Edge function**: `supabase/functions/send-user-hourly-memorization-reminders/` — **email** when `is_active`; **push** when `receive_push` + `device_tokens`; template from `admin_settings.user_hourly_memorization_reminder_template_key`. **`{{appLink}}`** = `APP_URL/?filter=memorize`. Invoked **second** by [`dispatch-user-reminders`](../supabase/functions/dispatch-user-reminders/index.ts). Retries transient PostgREST errors (including **`memorized_items`** for spotlight). Spotlight-template **email is skipped** when `memorized_items` load fails after retries ([`memorization-spotlight-reminder-email.ts`](../src/app/lib/memorization/memorization-spotlight-reminder-email.ts)). Same fail-closed template rules as the prayer hourly job (no `DEFAULT_*` fallback on a 504).
