...
- [`email-subscribers.component.ts`](src/app/components/email-subscribers/email-subscribers.component.ts) — list shell (search, pagination, toggles, tours). **Manual Entry** → **Add Subscriber** lives in [`admin-email-subscribers-add-form`](src/app/components/admin-email-subscribers-add-form/admin-email-subscribers-add-form.component.ts) (`type="button"`, `novalidate`, `tick` / `detectChanges`, inline **Adding…**). CSV import is [`admin-email-subscribers-csv-panel`](src/app/components/admin-email-subscribers-csv-panel/admin-email-subscribers-csv-panel.component.ts); rows render via [`admin-email-subscriber-row`](src/app/components/admin-email-subscriber-row/admin-email-subscriber-row.component.ts) emitting typed actions to the shell.

**Lower concern**:

- **Prayer Editor** ([`prayer-search.component.ts`](src/app/components/prayer-search/prayer-search.component.ts)): default change detection; **Create New Prayer** is handled by [`admin-prayer-editor-create-form`](src/app/components/admin-prayer-editor-create-form/admin-prayer-editor-create-form.component.ts) (`type="button"` `(click)="onCreateClick($event)"`). Expanded prayer rows render via [`admin-prayer-editor-card`](src/app/components/admin-prayer-editor-card/admin-prayer-editor-card.component.ts), which emits typed [`PrayerEditorCardAction`](src/app/lib/admin-prayer-editor-types.ts) events to the shell.
