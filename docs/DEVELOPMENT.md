...
- **Interactive preview**: [`info-feature-overview`](src/app/components/info-feature-overview/info-feature-overview.component.ts) composes [`info-mock-app-header`](src/app/components/info-mock-app-header/info-mock-app-header.component.ts) (Help, **Search** toggle, Settings, Pray, Request), a collapsed [`info-mock-search-bar`](src/app/components/info-mock-search-bar/info-mock-search-bar.component.ts) revealed by the header Search chip (`#info-mock-search-panel`), [`info-home-filter-preview-tabs`](src/app/components/info-home-filter-preview-tabs/info-home-filter-preview-tabs.component.ts) (**Church** → **Personal** → **Memorize**), and [`info-home-filter-preview-panels`](src/app/components/info-home-filter-preview-panels/info-home-filter-preview-panels.component.ts) (`@switch` for Current/Answered/Archived/Total/Prompts/Personal/Memorize; **Members** is Home-only when a Planning Center list is mapped). Explanation modals in [`info-preview-modals`](src/app/components/info-preview-modals/info-preview-modals.component.ts) on shared [`modal-shell`](src/app/components/modal-shell/modal-shell.component.ts). The host is a sibling of `.info-page-body` on [`info.component.html`](src/app/pages/info/info.component.html) so the book-page `zoom` / overflow frame cannot clip the overlay. Memorize **Add Verses** / **Bible Books** / **Recommended** chips emit `memorizeAction` explanations via [`info-preview-memorize-action-modal`](src/app/components/info-preview-memorize-action-modal/info-preview-memorize-action-modal.component.ts). Tapping the sample verse card emits `memorizePractice` and opens [`info-preview-memorize-practice-modal`](src/app/components/info-preview-memorize-practice-modal/info-preview-memorize-practice-modal.component.ts), a five-slide carousel (Type, Initials, Word, Reorder, Recite) whose light and dark screenshots follow the Info page theme (`html.dark`) from [`public/info/memorize-practice/light/`](public/info/memorize-practice/light/) and [`public/info/memorize-practice/dark/`](public/info/memorize-practice/dark/). Slide copy, theme path helper (`memorizePracticeSlideSrc`), and navigation helpers live in [`info-memorize-practice-preview.ts`](src/app/lib/info-memorize-practice-preview.ts). Preview types, `isPublicPreviewFilter()`, `isPublicAreaPreviewFilter()`, and `isMemorizePreviewFilter()` in [`info-home-filter-preview.types.ts`](src/app/lib/info-home-filter-preview.types.ts). Info preview TestBed specs auto-discover `info-*` component folders via [`info-preview-component-resources.spec-helper.ts`](src/app/components/info-preview-component-resources.spec-helper.ts).
...
- **Planning Center Members filter**: List/member sync, virtual member prayer cards, and targeted update reload are owned by [`home-planning-center.controller.ts`](src/app/services/home-planning-center.controller.ts); personal prayer reorder stays in [`home-personal-category.controller.ts`](src/app/services/home-personal-category.controller.ts). Members is a Church sub-filter, not a main tab.
...
- **Prayer reminders** (`help_prayer_reminders`): footer **Start guided tour** emits `startPrayerRemindersHelpSectionUiTour`; home calls [`startPrayerRemindersHelpSectionTour`](src/app/services/help-driver-tour.service.ts) (same Settings hooks). **Church** Next opens `[data-card-actions-trigger]` so the per-prayer reminder row (`#tour-prayer-reminder-bell`) is in the DOM. Reminders card: `tour-settings-prayer-reminders`; hour + **Add reminder** row: `tour-settings-prayer-reminder-controls` on [`UserSettingsComponent`](src/app/components/user-settings/user-settings.component.ts).
- **Feedback** (`help_feedback`): footer **Start guided tour** emits `startFeedbackHelpSectionUiTour`; home calls [`startFeedbackHelpSectionTour`](src/app/services/help-driver-tour.service.ts) (same Settings hooks). When [`environment.inAppFeedbackEnabled`](../src/environments/environment.ts) is true, Settings includes `tour-settings-feedback-section` with [`GitHubFeedbackFormComponent`](src/app/components/github-feedback-form/github-feedback-form.component.ts) (`tour-settings-feedback-type`, `tour-settings-feedback-details`), which invokes [`submit-feedback`](../supabase/functions/submit-feedback/index.ts) (Notion Site issues; MFA uses `verify_jwt: false` and `email_subscribers` validation like [`transcribe-audio`](../supabase/functions/transcribe-audio/index.ts)). When false, the feedback section and Feedback help tour are omitted ([`environment.prod.ts`](../src/environments/environment.prod.ts) until Notion is deployed on prod).
...
- **Location**: Settings modal → **Prayer reminders** section (above **Send Feedback**). Pick a time, **Add reminder**, or **Remove** on a slot. Times use the device IANA time zone when saving. Per-prayer once/daily/weekly reminders use **Add prayer reminder** in the card hamburger menu (see *Per-prayer item reminders* below).
...
- **Edge function**: `supabase/functions/send-user-hourly-prayer-reminders/` — same auth model as **`send-prayer-reminders`**. Sends **email** when `email_subscribers.is_active` is not false. Sends **push** when `receive_push` is true and a `device_tokens` row exists. **Both** when both apply. **Email** uses `email_templates` from **`user_hourly_prayer_reminder_template_key`**; **`{{appLink}}`** is home-only for the simple template, or **`APP_URL/?prayerId=`** for the spotlight pick when one exists. Push spotlight sends include **`prayerId`** for tap navigation ([`app.component.ts`](src/app/app.component.ts)). Retries transient PostgREST errors on settings, templates, due-now RPC, and batch reads. A failed `admin_settings` or primary `email_templates` read after retries is HTTP 500 (no send); `DEFAULT_*` is used only when that read succeeds and the key/row is empty or missing.
- **Scheduling (Supabase)**: pg_cron job **`invoke-dispatch-user-reminders`** (`*/15 * * * *` UTC) POSTs to [`dispatch-user-reminders`](../supabase/functions/dispatch-user-reminders/index.ts), which loads **`admin_settings` hourly template keys once**, pauses between phase invokes, retries a failed phase once, then runs this function **first**, then memorization hourly, then per-item reminders (sequential; avoids PostgREST stampede). When invoked from the dispatcher with `userHourlyPrayerReminderTemplateKey` in the body, this function skips its own `admin_settings` read. Migration [`20260914183000_dispatch_user_reminders.sql`](../supabase/migrations/20260914183000_dispatch_user_reminders.sql). See [SETUP.md](SETUP.md).
...
- **Edge**: `supabase/functions/send-user-prayer-item-reminders/` — deletes `once` rows after send; stamps `last_sent_at` for daily/weekly; skips + deletes inactive community/personal targets; injects latest update via `{{updateBlockHtml}}` (same CSS as spotlight). Invoked **third** by [`dispatch-user-reminders`](../supabase/functions/dispatch-user-reminders/index.ts). Retries transient PostgREST errors on the template, due-now RPC, subscribers, device tokens, and early status batches. A failed template load after retries is HTTP 500; the inline fallback is used only when the template row is missing with no error.
...
- **Edge function**: `supabase/functions/send-user-hourly-memorization-reminders/` — **email** when `is_active`; **push** when `receive_push` + `device_tokens`; template from `admin_settings.user_hourly_memorization_reminder_template_key` (or **`userHourlyMemorizationReminderTemplateKey`** in the POST body when invoked by [`dispatch-user-reminders`](../supabase/functions/dispatch-user-reminders/index.ts)). **`{{appLink}}`** = `APP_URL/?filter=memorize`. Invoked **second** by the dispatcher. Retries transient PostgREST errors (including **`memorized_items`** for spotlight). Spotlight-template **email is skipped** when `memorized_items` load fails after retries ([`memorization-spotlight-reminder-email.ts`](../src/app/lib/memorization/memorization-spotlight-reminder-email.ts)). Same fail-closed template rules as the prayer hourly job (no `DEFAULT_*` fallback on a 504).
- **Scheduling**: Same **`invoke-dispatch-user-reminders`** cron as prayer item reminders (see above); no separate memorization cron.
