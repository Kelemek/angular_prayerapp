...
- **Edge**: `supabase/functions/send-user-prayer-item-reminders/` — deletes `once` rows after send; stamps `last_sent_at` for daily/weekly; skips + deletes inactive community/personal targets; injects latest update via `{{updateBlockHtml}}` (same CSS as spotlight). Startup waits **5s**, then retries transient PostgREST errors on the template, due-now RPC, subscribers, device tokens, and early status batches. A failed template load after retries is HTTP 500; the inline fallback is used only when the template row is missing with no error.
...
- **Edge function**: `supabase/functions/send-user-hourly-memorization-reminders/` — **email** when `is_active`; **push** when `receive_push` + `device_tokens`; template from `admin_settings.user_hourly_memorization_reminder_template_key`. **`{{appLink}}`** = `APP_URL/?filter=memorize`. Startup waits **2.5s**. Same fail-closed template rules as the prayer hourly job (no `DEFAULT_*` fallback on a 504).
