...
- **Presentation card:** `prayer-display-card` — same Pray For / Prayed For / **{n} Praying** controls on congregation prayer slides in presentation mode, gated by the same user settings, admin feature flag, update-policy (`admin_settings.updates_allowed`), and card-type rules (community prayers only; not personal or Planning Center member cards).
- **Presentation loop:** [`PresentationSettingsService`](src/app/services/presentation-settings.service.ts) persists `loop` (default `true`). When `loop` is false, [`presentation.component.ts`](src/app/pages/presentation/presentation.component.ts) auto-play runs through all slides once, stops, and shows a completion overlay (same styling as the prayer timer end message). Closing the overlay calls `dismissPresentationComplete()`, which resets to the first slide and starts another single pass.
