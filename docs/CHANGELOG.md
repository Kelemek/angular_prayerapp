...
- **Readability (booklet)**: Each half-letter panel uses **0.4in padding on all four sides**, **larger** type (section titles ~**16px**, body/description **12px**, updates **11px**), and up to **four** prayer cards per panel so text fits without clipping. **Prayer cards** on booklet inner pages use the **same** borders, **status** left accent (**current** / **answered** / **archived**), typography, markdown list/quote rules, and **updates** box styling as **Print Prayers** (`generatePrintableHTML()`), including the **Updates (n):** header before the shown update, implemented in **`generateSaddleStitchBookletHTML()`** in [`print.service.ts`](src/app/services/print.service.ts). **Front cover**: large rounded PWA [**/icons/icon-512.png**](public/icons/icon-512.png) app mark and **Prayer List** title (~**32px**) with date range (**no** printed “generated at” line); a horizontal rule (**`<hr>`**) separates the main header from the **bottom** strip; that strip has **Download the app** + blurb (emphasized **Join us in prayer**, Sundays **6–6:25 PM**; **~17px** / **~14px**, left) and a larger **`/info` QR** (~**1.2in / ~135px**, right); the cover **fills the panel content height** so that strip anchors to the **physical bottom** of the front page. **Back cover** (physical last page): optional **church logo** (**BrandingService** light logo when **Use logo** is on), **bottom-centered**. The standard **Print Prayers** HTML footer is unchanged.
- **Blank / imposition**: Padding pages insert **before** the back cover when needed so the **last reader page stays the outer back cover** after folding (blank cells have **no** placeholder text).
