...
### Admin Prayer Editor card — template split
- [`admin-prayer-editor-card`](src/app/components/admin-prayer-editor-card/admin-prayer-editor-card.component.ts) (~1k-line inline template) split into header, expanded panel, edit form, view details, updates list, and add-update child components; parent shell keeps tour anchors and delegates `ViewChild` flush/reset via child panels.
