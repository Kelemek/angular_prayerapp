...
- [`prayer-card`](src/app/components/prayer-card/prayer-card.component.ts) hoists Pray For visibility settings from Home (same as prompt cards) and subscribes once for cooldown hours instead of per-card async pipes in [`prayer-card-actions-row`](src/app/components/prayer-card/prayer-card-actions-row.component.html). **Show all updates** and add-update expand call [`scheduleHomePrayerVirtualScrollRemeasure`](src/app/lib/home-prayer-virtual-scroll.ts) so autosize rows remeasure when inner card height changes.
