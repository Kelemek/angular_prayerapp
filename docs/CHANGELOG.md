...
- **Behavior**: When Admin → **Settings** → **Email** uses the **random recent prayer** template option, the hourly Edge Function [`send-user-hourly-prayer-reminders`](supabase/functions/send-user-hourly-prayer-reminders/index.ts) picks a random **current** community prayer (`prayers`: approved, `status = current`) or personal prayer (`personal_prayers`: not **Answered**) that was **created or updated in the last 14 days**, preferring a **different** prayer than the last email when more than one qualifies. Spotlight queries use PostgREST **unquoted** ISO timestamps in `.or()` (same pattern as [`prayer.service.ts`](src/app/services/prayer.service.ts) `getPrayersByMonth`) so the 14-day window is applied reliably. Template variables `{{spotlightPrayerKind}}`, `{{spotlightPrayerTitle}}`, `{{spotlightPrayerFor}}`, `{{spotlightPrayerDescription}}` are filled (empty when none); **`{{updateContent}}`** is the **most recent approved** community update or **latest** personal update (`personal_prayer_updates`), with **`{{spotlightLatestUpdateHtml}}`** / **`{{spotlightUpdateTextSection}}`** for default layouts; HTML bodies use escaped text. **Push** includes a truncated snippet of the latest update when present.
