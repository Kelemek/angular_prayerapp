...
- Long-pressing a category chip to rename no longer highlights **Cancel** / **Save** or the **Category name** label when the modal appears under your finger: the release gesture is swallowed, native selection is cleared, static modal copy is non-selectable, and the rename input is focused after lift ([`personal-category-long-press.ts`](src/app/lib/personal-category-long-press.ts), [`personal-category-rename-modal`](src/app/components/personal-category-rename-modal/personal-category-rename-modal.component.ts)).
