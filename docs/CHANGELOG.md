...
- **Production builds** keep `inAppFeedbackEnabled: false` in [`environment.prod.ts`](../src/environments/environment.prod.ts) until `submit-feedback` is deployed on prod; the Settings feedback section is omitted entirely (no broken form). Test/dev use `true` in [`environment.ts`](../src/environments/environment.ts). Feedback and App Settings help tours skip feedback steps when the flag is false; the full guided tour skips `help_feedback` via [`dispatchHomeHelpSectionTour`](src/app/lib/home-help-tour-dispatch.ts). Help hides **Start guided tour** on Send Feedback when the flag is off ([`helpSectionHasUiTour`](src/app/lib/help-section-ids.ts)). Recite feedback help does not scroll to a missing section when feedback is off ([`home-modal.controller.ts`](src/app/services/home-modal.controller.ts)).
